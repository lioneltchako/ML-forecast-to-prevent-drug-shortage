...
# ANONYMIZATION NOTE
# ─────────────────────────────────────────
# All data on this page is fully synthetic.
# It was generated to match the statistical
# properties of the real dataset:
#   - Same number of products (43)
#   - Same franchise distribution
#   - Same seasonality pattern
#   - Same order of magnitude for sales volumes
#   - Same COVID-19 impact shape
#   - Same coefficient of variation distribution
#
# No real sales figures, product names, or
# identifiers from the original study are used.
# ─────────────────────────────────────────
...
def generate_product_sales(
    ...
    """
    Generate synthetic monthly sales for one product.

    Parameters
    ----------
    base_volume  : average monthly units
    cv           : coefficient of variation (controls volatility)
    franchise    : used to apply franchise-specific seasonality
    has_covid_dip: whether to apply COVID-19 drop (2020-01 to 2021-07)
    """
...
for franchise, meta in FRANCHISES.items():
    for _ in range(meta["products"]):
        ...
        cv     = np.random.choice(
            [np.random.uniform(0.05, 0.45),   # stable
             np.random.uniform(0.50, 0.95),   # variable
             np.random.uniform(1.00, 1.60)],  # high volatility
            p=[0.77, 0.07, 0.16],
        )
...
monthly_avg["month_name"] = pd.to_datetime(
    monthly_avg["month"], format="%m"
).dt.strftime("%b")
...
WARNING = "#BA7517"
NEUTRAL = "#888780"
FRANCHISE_COLORS = [m["color"] for m in FRANCHISES.values()]
...
def render():
    ...
    st.markdown(
        "Exploring what pharmaceutical sales data looks like "
    ...
    with col_f1:
        fig_f = px.pie(
            ...
            names="franchise",
            values="share",
            color_discrete_sequence=FRANCHISE_COLORS,
            hole=0.45,
        )
    ...
    with col_f2:
        ...
        for _, row in franchise_dist.iterrows():
            st.markdown(
                f"**{row['franchise']}** — "
                f"{row['products']} products · "
                f"{row['share']}% of total sales"
            )
    ...
    st.caption(
        "Total synthetic sales across all 43 products. "
        "The COVID-19 period is highlighted."
    )
    ...
    with col_t1:
        st.info(
            "**Upward trend** — Overall sales grow steadily, "
            "reflecting portfolio expansion and market growth.",
            icon="📈",
        )
    with col_t2:
        st.warning(
            "**COVID-19 dip** — A significant drop in 2020–2021 "
            "due to supply chain disruptions and demand shifts.",
            icon="🦠",
        )
    st.divider()
    ...
    st.caption(
        "A recurring annual pattern is visible: "
        "sales peak mid-year and dip in early months."
    )

    colors = [
        WARNING if row["avg_sales"] == monthly_avg["avg_sales"].max()
        else PRIMARY
    ...
    fig_seas = go.Figure(go.Bar(
        ...
        y=monthly_avg["avg_sales"].round(0),
        marker_color=colors,
        text=monthly_avg["avg_sales"].round(0).astype(int),
    ...
    st.markdown(
        "> This seasonality pattern is a key signal that the model must "
        "learn. It is captured using **trigonometric features** "
        "(sin/cos of month) in the feature engineering step."
    )
    st.divider()
    ...
    st.caption(
        ...
        "unpredictable each product's sales are."
    )
    ...
    with col_cv2:
        ...
        st.success(
            "**Stable products (CV < 50%)** — "
            "Predictable demand. "
            "~33 products fall here.",
        )
        st.warning(
            "**Variable products (50–100%)** — "
        ...
        st.error(
            ...
            "~7 products. "
            "These drive most of the model error.",
        )
    st.divider()
    ...
    with col_sel2:
        products_in_franchise = df[
            df["franchise"] == selected_franchise
        ]["product_id"].unique()
        selected_product = st.selectbox(
            "Product",
            options=sorted(products_in_franchise),
        )
    ...
    if product_cv < 50:
        volatility_label, volatility_color = "Stable",        SUCCESS
    elif product_cv < 100:
        volatility_label, volatility_color = "Variable",      WARNING
    else:
        volatility_label, volatility_color = "High volatility", DANGER

    col_pm1, col_pm2, col_pm3 = st.columns(3)
    col_pm1.metric("Franchise",   selected_franchise)
    col_pm2.metric("CV",          f"{product_cv}%", volatility_label)
    col_pm3.metric(
        "Avg monthly sales",
        f"{int(product_df['sales'].mean()):,} units",
    )
    ...
    fig_prod.add_trace(go.Scatter(
        ...
        mode="lines+markers",
        marker=dict(size=4),
        line=dict(color=FRANCHISE_COLORS[list(FRANCHISES).index(selected_franchise)],
                  width=2),
    ...
    # ── Outlier handling ──────────────────
    st.markdown("### How outliers were handled")
...
For each product, outliers were identified using
the interquartile range:

- Lower bound = Q1 − 1.5 × IQR
- Upper bound = Q3 + 1.5 × IQR

Values outside these bounds were flagged.
""")
...
Rather than deleting flagged values,
they were replaced using **K-Nearest Neighbors
imputation** (k = 5 neighbors).
...
""")
    ...
    with st.expander("Why not simply delete outliers?"):
        st.markdown("""
Deleting outliers would reduce the dataset size,
which is already limited for some products
(minimum 39 months of history).
...
- It avoids introducing a constant (e.g. mean imputation)
  which would artificially reduce variance
""")
    st.divider()
    ...
    st.markdown("""
Three properties of this dataset directly shape
the modelling decisions made in the next steps:
""")
    ...
    with col_m1:
        st.info(
            "**Seasonality is real** — "
            "Monthly patterns are consistent enough to be learned. "
            "→ Trigonometric features needed.",
            icon="📅",
        )
    with col_m2:
        st.warning(
            "**COVID-19 is an anomaly** — "
            "It must be explicitly flagged, not learned as a trend. "
            "→ Binary covid_period feature needed.",
            icon="🦠",
        )
    with col_m3:
        st.error(
            "**High volatility products exist** — "
            "A single model will struggle with them. "
            "→ Per-product training strategy needed.",
            icon="⚠️",
        )

    st.caption(
        "⚠️ All data on this page is synthetic and generated "
        "solely for illustration purposes. "
        "Statistical properties match the real dataset; "
        "no actual figures are reproduced."
    )
